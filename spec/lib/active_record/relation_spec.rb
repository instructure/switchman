    ...
    describe Relation do
      ...
      describe "#records" do
        ...
        it "raises an error if you have a sort order on a multi-shard query" do
          expect do
            User.where(id: [@user1.id, @user2.id]).order(::Arel.sql("id")).to_a
          end.to raise_error(OrderOnMultiShardQuery)
        end
        ...
        it "implements cross-shard limit with count" do
          expect(User.where(id: [@user1.id, @user2.id]).limit(1).count).to eq 1
        end
        ...
        it "implement cross-shard limit on non-boundary with count" do
          @user3 = @shard1.activate { User.create! }
          expect(User.where(id: [@user1.id, @user2.id, @user3.id]).limit(2).count).to eq 2
        end
        ...
        context "with cross-shard sorting" do
          ...
          it "sorts a pluck" do
            expect(User.where(id: [@user1.id,
                                   @user2.id]).order(id: :desc).pluck(:id)).to eq [@user1,
                                                                                   @user2].map(&:id).sort.reverse
          end
        end
      end

      describe "#update_all" do
        it "activates the correct shard for the query" do
      ...
      describe "#new" do
        it "infers the scope's shard" do
